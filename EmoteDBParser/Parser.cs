...
public static class Parser
{
        ...
        int rowEnd)
    {
        ...
        result.Icon = iconTag >= 0
            ? ReadSoftObjectPathProperty(uexp, iconTag, package.Names)
            : null;
    ...
    private static string? ReadSoftObjectPathProperty(
        byte[] data,
        int tag,
        List<string> names)
    {
        ...
        int nameIndex = ReadInt32(data, tag + 25);

        if (nameIndex < 0 || nameIndex >= names.Count)
            return null;

        string path = names[nameIndex];

        return path.StartsWith("/Game/", StringComparison.Ordinal)
            ? CleanAssetPath(path)
            : null;
    }
